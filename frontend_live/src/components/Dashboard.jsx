  ...
  fetch(`/api/app-state?userId=${userInfo.sub}`)
    ...
    .then(data => {
      if (data.success && data.state) {
        // ✅ FIX: Backend now returns parsed object, no need to parse again
        const savedState = data.state;

        // ✅ Validate that savedState is actually an object
...
useEffect(() => {
  const handleSync = (payload) => {
    ...
    // ✅ FIX 1: Unwrap the payload correctly (handles both payload.state and direct payload)
    ...
    // ✅ FIX 2: Always update selectedModels (no guard needed)
    // Panel visibility is controlled by (isTrading || tradingStopped) condition in JSX
    setSelectedModels(incoming.selectedModels || []);

    setStartingValue(String(incoming.startingValue ?? "100")); // ✅ normalize to string
    setStopLoss(incoming.stopLoss || "");
    setProfitTarget(incoming.profitTarget || "");
    setIsTrading(!!incoming.isTrading); // ✅ normalize to boolean
    setTradingStopped(!!incoming.tradingStopped); // ✅ normalize to boolean
    setStopReason(incoming.stopReason || "");
    setFinalProfitLoss(incoming.finalProfitLoss || null);
    setInitialValues(incoming.initialValues || {});
    setUpdateSpeed(incoming.updateSpeed || "1500");
    setIsMockTrading(incoming.isMockTrading !== false);

    //addLog('🔄 Settings synced from another device', 'info');
  };

  socket.on('app_state_sync', handleSync);

  return () => socket.off('app_state_sync', handleSync);
}, []); // ✅ FIX 3: Empty dependency array prevents stale closure issues
...
useEffect(() => {
  ...
  if (isTrading) {
    interval = setInterval(() => {
      checkRiskManagement(); // ✅ still runs only while trading
    }, 5000);
  }
...
const handleStartTrading = async () => {
  ...
  if (userInfo?.sub) {
    const stateToSave = {
      selectedModels,
      startingValue: String(sv), // ✅ Keep as string for consistency with UI
      stopLoss,
      profitTarget,
      isTrading: true,
    ...
    <div className="dashboard" style={{ minHeight: '100vh', paddingBottom: '40px' }}>
      ...
      >
        ...
        <div style={{ marginTop: '15px' }}>
          ...
          {console.log("Panel visible:", showMonitoringPanel, "isTrading:", isTrading, "tradingStopped:", tradingStopped)}

          {console.log("🚨 PANEL JSX EXECUTED")}
          {true && (
  <>
    {/* 🔍 DEBUG: Log panel render state */}
    {console.log("🔍 Rendering Monitoring Panel:", { 
      isTrading, 
      tradingStopped, 
      selectedModelsCount: selectedModels.length,
      models: selectedModels,
      modelsLatestKeys: Object.keys(modelsLatest)
    })}
