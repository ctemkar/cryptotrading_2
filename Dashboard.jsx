  ...
  return (
    <div className="dashboard">
      ...
      {/* Debug Panel for History Data */}
        ...
        >
          <h3 style={{ marginTop: 0, marginBottom: '10px' }}>🔍 Debug: Models History Data</h3>
          <div style={{ fontSize: '12px', fontFamily: 'monospace' }}>
            <div><strong>modelsHistory keys:</strong> {JSON.stringify(Object.keys(modelsHistory || {}))}</div>
            <div><strong>modelsLatest keys:</strong> {JSON.stringify(Object.keys(modelsLatest || {}))}</div>
            <div><strong>History data points:</strong> {
              Object.keys(modelsHistory || {}).length > 0 
                ? Object.keys(modelsHistory).map(key => `${key}: ${modelsHistory[key]?.length || 0} points`).join(', ')
                : '❌ No history data yet - waiting for model_update events'
            }</div>
            {Object.keys(modelsHistory || {}).length > 0 && (
              <div style={{ marginTop: '10px' }}>
                <strong>Sample history entry (first model, last 2 points):</strong>
                <pre style={{ background: 'white', padding: '10px', borderRadius: '4px', overflow: 'auto', maxHeight: '200px' }}>
                  {JSON.stringify(modelsHistory[Object.keys(modelsHistory)[0]]?.slice(-2), null, 2)}
                </pre>
              </div>
            )}
            {Object.keys(modelsHistory || {}).length === 0 && (
              <div style={{ marginTop: '10px', padding: '10px', background: '#ffebee', borderRadius: '4px', color: '#c62828' }}>
                ⚠️ <strong>No history data!</strong> Your backend might not be sending 'model_update' events, or the socket connection is not working properly.
              </div>
            )}
          </div>
        </div>
