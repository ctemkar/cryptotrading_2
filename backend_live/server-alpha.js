...
// ✅ ADD THIS LINE HERE (at the top of the file)
...
async function initDatabase() {
  try {
    ...
    // ✅ NEW: Track Gemini connection status per user
    await db.query(`
      CREATE TABLE IF NOT EXISTS gemini_connection_status (
        user_id VARCHAR(255) PRIMARY KEY,
    ...
    console.log("✅ gemini_connection_status table ready");
...
// ✅ Helper to save log to DB AND broadcast to all devices
function addLog(userId, type, message, data = {}) {
  ...
  // 1. Save to Database
  db.query(
    `INSERT INTO trade_logs_archive (user_id, message, type, metadata, timestamp) VALUES (?, ?, ?, ?, ?)`,
    [userId, message, type, JSON.stringify(data), timestamp]
  ).catch((err) => {
    console.error("❌ Database Log Error:", err.message);
  });

  // 2. Broadcast to all of the user's connected devices
  io.to(`user:${userId}`).emit('log:new', logEntry);
  
  console.log(`📢 Log Broadcasted to user:${userId}: ${message}`);
}
...
async function openLiveGeminiPosition({ modelId, modelName, symbol, amount, price, side, userId }) {
  const key = livePosKey(modelId, symbol);
  const position = {
    modelId,
    modelName,
  ...
  liveGeminiPositions[key] = position;
  console.log('📌 [LIVE] Opened Gemini position:', position);

  // ✅ Persist to database
  if (userId) {
    try {
      await db.query(
        'INSERT INTO gemini_open_positions (user_id, model_id, model_name, symbol, side, amount, entry_price) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [userId, modelId, modelName, symbol.toLowerCase(), side.toUpperCase(), amount.toString(), price.toString()]
      );
  ...
  if (userId) {
    io.to(`user:${userId}`).emit('position_opened', position);
    addLog(userId, 'success', `🚀 ${modelName} opened ${side} on ${symbol.toUpperCase()} @ $${price}`);
  } else {
    io.emit('position_opened', position); // fallback for legacy code
  }
}
...
async function closeLiveGeminiPositionAndRecord({
  ...
  if (remaining <= 0.00000001) {
    ...
    if (userId) {
      try {
        await db.query(
          'DELETE FROM gemini_open_positions WHERE user_id = ? AND model_id = ? AND symbol = ?',
          [userId, modelId, symbol.toLowerCase()]
        );
        console.log('✅ Position removed from database');
      } catch (err) {
    ...
    if (userId) {
      try {
        await db.query(
          'UPDATE gemini_open_positions SET amount = ? WHERE user_id = ? AND model_id = ? AND symbol = ?',
          [remaining.toString(), userId, modelId, symbol.toLowerCase()]
        );
        console.log('✅ Position updated in database (remaining:', remaining, ')');
      } catch (err) {
  ...
  // ✅ Broadcast only to this user's devices
  if (userId) {
    io.to(`user:${userId}`).emit('position_closed', {
      model_id: modelId,
...
function startGeminiTradesPolling() {
  ...
  geminiTradesIntervalId = setInterval(async () => {
    ...
    for (const symbol of symbols) {
      try {
        await axios.get(`http://localhost:${PORT}/api/gemini/market-trades`, {
...
app.put('/api/app-state', async (req, res) => {
  try {
    ...
    // ✅ Broadcast state change to all user's devices EXCEPT the sender
    if (socketId && socketsById.has(socketId)) {
      ...
      console.log(`📢 State synced to other devices for user:${userId} (excluding sender ${socketId})`);
    } else {
      io.to(`user:${userId}`).emit('app_state_sync', { state: finalState, version: newVersion });
      console.log(`📢 State synced to all devices for user:${userId}`);
    }
...
app.post('/api/logs/archive', async (req, res) => {
  try {
    ...
    // ✅ Broadcast log to all user's devices using addLog
    addLog(userId, type || 'info', message, metadata || {});
...
// ✅ NEW: Get Gemini connection status
app.get('/api/gemini/connection-status', async (req, res) => {
  try {
    const { userId } = req.query;
    if (!userId) return res.status(400).json({ success: false, error: 'Missing userId' });

    const [rows] = await db.query(
      'SELECT is_connected, last_connected_at FROM gemini_connection_status WHERE user_id = ?',
      [userId]
    );

    return res.json({
      success: true,
      isConnected: rows.length > 0 && rows[0].is_connected === 1,
      lastConnectedAt: rows[0]?.last_connected_at || null
    });
  } catch (err) {
    console.error('❌ Error checking connection status:', err);
    return res.status(500).json({ success: false, error: 'Failed to check connection status' });
  }
});
...
app.delete('/api/gemini/credentials', async (req, res) => {
  try {
    ...
    // ✅ NEW: Mark user as disconnected
    await db.query('UPDATE gemini_connection_status SET is_connected = 0 WHERE user_id = ?', [userId]);
...
app.post("/api/gemini/balances", async (req, res) => {
  try {
    ...
    // ✅ Mark user as connected after successful balance fetch
    await db.query(
      'INSERT INTO gemini_connection_status (user_id, is_connected, last_connected_at) VALUES (?, 1, NOW()) ON DUPLICATE KEY UPDATE is_connected = 1, last_connected_at = NOW()',
      [userId]
    );
...
app.get('/api/gemini/open-positions', async (req, res) => {
  try {
    const { userId } = req.query;

    // ✅ If userId provided, load from database (for hydration)
    if (userId) {
      const [rows] = await db.query(
        'SELECT * FROM gemini_open_positions WHERE user_id = ?',
        [userId]
      );
      ...
      positions.forEach(pos => {
        const key = livePosKey(pos.modelId, pos.symbol);
        liveGeminiPositions[key] = pos;
      });

      return res.json({ success: true, positions });
    }

    // ✅ Fallback: return in-memory positions
    ...
    for (const pos of positionsToClose) {
      try {
        ...
        const closingInfo = await closeLiveGeminiPositionAndRecord({
          ...
          amount: executed,
          exitPrice: exitPrice,
          userId, // ✅ Pass userId for broadcasting
        });
...
io.on("connection", socket => {
  ...
  socket.on("join_user_room", (userId) => {
    ...
    console.log(`📊 User ${userId} now has ${count} connected socket(s)`);
  });
